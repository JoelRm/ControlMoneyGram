var app_ReporteOperacionesCalculadora = (function (win, doc) {

	const data = {
		urlGenerarReporteCalculadora: '/Reporte/GenerarReporteCalculadora'
	}

	function init() {
		document.getElementById("btnExportarOperacionesCalc").style.display = "none";
		formatoRangoFecha();
        document.getElementById("btnBuscarOperacionesCalc").addEventListener("click", listarOperacionesCalculadora);
        document.getElementById("btnExportarOperacionesCalc").addEventListener("click", ExportarExcel);
            ...
            success: function (data) {
                ...
                if (data.lstReporte.length > 0) {
                    for (var i = 0; i < data.lstReporte.length; i++) {
                        htmls += '<tr>';
                        htmls += '<td>' + data.lstReporte[i].Operacion + '</td>';
                        htmls += '<td>' + data.lstReporte[i].Resultado + '</td>';
                        htmls += '<td>' + data.lstReporte[i].Comentario + '</td>';
                        htmls += '<td>' + data.lstReporte[i].HoraCreacion + '</td>';
                        htmls += '<td>' + data.lstReporte[i].UsuarioCreacion + '</td>';
                        htmls += '</tr>';
                    }
                    htmls += "</tbody>";
                    htmls += "</table>";
	...
	function listarOperacionesCalculadora() {
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (rpta.length > 0) {
					crearTabla(rpta);
					document.getElementById("btnExportarOperacionesCalc").style.display = "block";
				} else {
