var app_Reporte = (function (win, doc) {
    ...
    function ExportarExcel() {
        ...
        htmls += "<table border='2px'>"
        htmls += "<thead>";
        htmls += "<thead><th width='1%'>Operación</th><th width='1%'>Movimiento</th><th width='1%'>Salida</th><th width='1%'>Ingreso</th><th width='1%'>Tipo Cambio</th><th width='2%'>Caja (S/.)</th><th width='2%'>Caja ($)</th><th width='2%'>Caja (€)</th><th width='2%'>Usuario</th><th width='1%'>Hora</th><th width='1%'>Estado</th><th width='1%'>Observación</th></tr></thead>";
        htmls += "<tbody>";
        $.ajax({
            type: "POST",
            ...
            success: function (data) {
                if (data.lstReporteCajaChica.length > 0) {
                    for (var i = 0; i < data.lstReporteCajaChica.length; i++) {
                        ...
                        htmls += '<td width="1%">' + data.lstReporteCajaChica[i].Comentario + '</td>';
                        htmls += '</tr>';
                    }
                    htmls += "</tbody>";
                    htmls += "</table>";
